...
//    var selectedOption = select.options[select.selectedIndex];
    dataFieldToGraph = (select.options[select.selectedIndex]).value;
...
function changeGraphText(select,key){
    switch(key){
    case 1:
	graphText1 = (select.options[select.selectedIndex]).value;
	console.log("GraphText1:"+graphText1);
	break;
    case 2:
	graphText2 = (select.options[select.selectedIndex]).value;
	console.log("GraphText2:"+graphText2);
	break;
    case 3:
	graphText3 = (select.options[select.selectedIndex]).value;
	console.log("GraphText3:"+graphText3);
	break;
    }
    replot();
}
...
function plot (field) { 
	...
	var text= svg.selectAll("text")
	    ...
	    .enter()
	    .append("text")
	    .text(function(d) { return d[graphText1]+" / "+d[graphText2]+" / "+d[graphText3] })
	...
	var text= svg.selectAll("text")
	    ...
	    .attr("x", function(d, i) { return 0 })
	    .attr("y", function(d,i) { return i*barSpacer+textHeight}) 
//	    .text(function(d) { return d.Age+"/"+d.Place+" "+d[graphText] })
...
//	    .text(function(d) { return d.Age+"/"+d.Place })
	    .text(function(d) { return d.Age+"/"+d.Gender })
	    .attr("x", function(d, i) { return 0 })
	    .attr("y", function(d,i) { return i*barSpacer+textHeight}) 
...
function replot () { 
    ...
    var xScale = d3.scale.linear()
	.domain([0,d3.max(data2,function(d) { return d[dataFieldToGraph];})])
	.range([0,width]);

//    console.log('d3.max:'+d3.max(data2,function(d) { return d[dataFieldToGraph];}));

//update only the dataFieldToGraphs that change
    var rect = svg.selectAll("rect")
	.data(data2)
	.attr("width", function(d) { var val = xScale(parseInt(d[dataFieldToGraph],10)); console.log(typeof val); return isNaN(val)?0:val;  })
	.attr("fill", function(d) {return genderColor(d)})
...
function changeSortField(select,key) {
    switch(key) {
    case 0:
	dataSortReverseOrder = select.checked;
	break;
    case 1:
	dataSortField = (select.options[select.selectedIndex]).value;
	break;
    }
    
    data2.sort(function(a,b) { 
	var value=0 	//equal, should never happen for certain fields
	if (a[dataSortField] > b[dataSortField]) { 
	    value=1;
	}
	if (a[dataSortField] < b[dataSortField]) {
	    value=-1;
	}
	if (dataSortReverseOrder === true) {
	    return value*-1;
	} else {
	    return value; ;
	}
...
// // ugg to be json each field must be in quotes : ie. "Age":23, "Bib_No":12.... and not Age:23, Bib_No:12
// var data2;
// load data from json file CAVEATE EMPTOR: JS objects != JSON
...
//window.onload=createMenu("Swim_Rnk"); //we were attempting to self create the menu of fields
